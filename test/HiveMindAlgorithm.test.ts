...
test('HiveMindAlgorithm: reinforce decreases weight on negative reward', () => {
  const algo = new HiveMindAlgorithm(0.1);
  algo.reinforce({ featureKey: 'gaming', reward: -1 });
  assert.ok(algo.getWeight('gaming') < 1.0);
});

test('HiveMindAlgorithm: applyWeights clamps result to [0, 100]', () => {
  const algo = new HiveMindAlgorithm(0.1);
  // Reinforce heavily to push weight high
  for (let i = 0; i < 100; i++) {
    algo.reinforce({ featureKey: 'extreme', reward: 10 });
  }
  const clamped = algo.applyWeights(99, 'extreme');
  assert.equal(clamped, 100);
});

test('HiveMindAlgorithm: getMatrix returns a defensive copy', () => {
  const algo = new HiveMindAlgorithm(0.1);
  algo.reinforce({ featureKey: 'test', reward: 2 });
  const matrix = algo.getMatrix();
  assert.equal(matrix.updateCount, 1);
  assert.equal(matrix.weights['test'], 1.2);

  // Mutating the returned object should not affect the internal state.
  (matrix as { weights: Record<string, number> }).weights['test'] = 999;
  assert.equal(algo.getWeight('test'), 1.2);
});

test('HiveMindAlgorithm: unseen features default to weight 1.0', () => {
  const algo = new HiveMindAlgorithm();
  assert.equal(algo.getWeight('never-seen'), 1.0);
});

// ─── MatchMaker + HiveMind integration ───────────────────────────────────────
