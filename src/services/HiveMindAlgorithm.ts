...
export class HiveMindAlgorithm {
  ...
  /**
   * @param initialWeights  Seed weights keyed by feature name (default: empty,
   *                        so every unseen feature starts at 1.0).
   * @param learningRate    Step size for each weight update (default 0.1).
   */
  constructor(
    initialWeights: Record<string, number> = {},
    private readonly learningRate = 0.1
  ) {
    this.matrix = { weights: { ...initialWeights }, updateCount: 0 };
  }

  /**
   * Convert a sentiment array into a score boost in the range [0, 20].
   *
   * Weights: valence 50 %, arousal 30 %, dominance 20 %.
   */
